...
Definition hexToProgram (s : string) : (option EVMException) * (option Program) :=
  match (BITStoProgram (fromHex s) nil) with
    | None => (Some BadFormProgram, None)
    | Some p => if checkwellformedProgram (rev p) then
                  (None, Some (rev p))
                else
                  (Some BadFormProgram, None)
  end.
...
Compute (
    match pro1 with
      | (_, None) => "Ill-formed program"%string
      | (_, Some p) => programToString p
    end
  ).

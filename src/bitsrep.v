...
(* We represent n-bit words by a tuple of booleans. 
 ...
 For example number 8 as an EVMWORD is the tuple:
 0001 0000 ... 0000 - 32 bytes *)
...
(* From tuple of BYTEs to BITS n, preserve the order in b or the head is the least-significant byte *)
...
(*--------------------------------------------------------------------------
 from BITS n to Hex string in Big Ediant representation
 --------------------------------------------------------------------------*)
Fixpoint toHex {n} :=
...
(* BYTEs to Hex with spaces like 01 0A 0C, preserve the order in seq *)

Fixpoint bytesToHexAux (b : seq BYTE) (res : string) :=
  match b with
    | b::bs => bytesToHexAux bs (res ++ " " ++ (toHex b))
    | nil => res
  end.

(* Note that the string representation in BE *)
...
Fixpoint addressesToHexAux (a : seq ADDRESS) (res : string) :=
  match a with
    | x::xs => addressesToHexAux xs (res ++ " " ++ (toHex x))
    | nil => res
  end.
...
Fixpoint evmwordsToHexAux (evmw : seq EVMWORD) (res : string) :=
  match evmw with
    | x::xs => evmwordsToHexAux xs (res ++ " " ++ (toHex x))
    | nil => res
  end.
...
(*-------------------------------------------------------------------------------
 BITS n to string preserving the order of bits
 -------------------------------------------------------------------------------*)
Definition appendBitOnString b s :=
  (if b then "1" ++ s else "0" ++ s)%string.

Fixpoint toString {n} :=
  match n return BITS n -> string with
  | 0 => fun bs => EmptyString
  | _ => fun bs => appendBitOnString (thead bs) (toString (behead_tuple bs))
...
Compute (toString fortytwo2).
Compute (toString fortytwo1).
Compute (toHex fortytwo2).

(* tuple of 5 BYTEs *)
Example bs : 5.-tuple BYTE := [tuple (#5:BYTE); (#4:BYTE); (#3:BYTE); (#2:BYTE); (#1:BYTE)].
Compute (bytesToHex bs).

Example fbs := fromBytes bs.
Compute (toString fbs).
Compute (toHex fbs).
